...
ggplot(elections_historic, aes(x = popular_pct, y = ec_pct,
  ...
  geom_text_repel(data = subset(elections_historic,
                                year %in% "2000" |
                                  year %in% "1876")) +
  annotate(geom="text", x = .3, y = .93,
           label = "The 2000 and 1876 elections both faced partisan disputes over vote counts",
           hjust=0, 
           fontface="italic", 
           color = "purple") +
  annotate(geom = "segment", 
  ...
  labs(x = "Winner's share of popular vote", 
       ...
       title = "Presidential Elections: Popular & Electoral College Margins", 
       subtitle = "1824-2016",
       caption = "Bush's 2000 election was decided by the Supreme Court, while Hayes's was decided by Congress") # adds a caption
